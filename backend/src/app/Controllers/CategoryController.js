...
export const createCategory = async(req,res)=>{
    try {
        ...
        if(error.code === 11000){
            ...
            return res.status(400).json({message:`the ${field} is already created, try another ${field}`})
        }
...
export const getAllCategories = async(req,res)=>{
    try {
        ...
        return res.status(200).json({message:`Successfully retrieved all categories data`,data:categories})
    } catch (error) {
