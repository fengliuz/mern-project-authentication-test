...
import { 
  Package, PlusCircle, Search, AlertTriangle, 
  MoreVertical, Layers, Box, Info,
  SearchCheckIcon,
  SearchCode
} from "lucide-react";
...
const ProductManagerPage = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      toast.error("Gagal memuat produk",error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col gap-6 p-4 lg:p-8 min-h-screen bg-base-200/50">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-primary mb-1">
            ...
            <span className="text-xs font-black tracking-widest uppercase opacity-70">Inventory System</span>
          </div>
        ...
        <div className="flex gap-2">
          <button 
            ...
            className={`btn ${isFormOpen ? 'btn-error' : 'btn-primary'} shadow-lg group`}
          >
            {isFormOpen ? "Cancel" : <><PlusCircle size={20} className="group-hover:rotate-90 transition-transform" /> New Product</>}
          </button>
        </div>
      </div>
      ...
      {isFormOpen && (
        <div className="flex justify-center animate-in zoom-in-95 duration-300">
           <AddProduct onSuccess={() => { setIsFormOpen(false); fetchProducts(); }} />
        </div>
      )}

      {/* SEARCH BAR: Fokus ke Glassmorphism */}
      <div className="relative group max-w-2xl flex items-center">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 opacity-30 group-focus-within:opacity-100 group-focus-within:text-primary transition-all" size={20} />
        <input 
          type="text" 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredProducts.map((p) => {
            ...
            return (
              <div 
                key={p._id} 
                className={`group card bg-base-100 shadow-sm hover:shadow-xl transition-all border-b-4 ${isLowStock ? 'border-error' : 'border-secondary'}`}
              >
                <div className="card-body p-5">
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="dropdown dropdown-end">
                      <button tabIndex={0} role="button" className="btn btn-ghost btn-xs btn-circle"><MoreVertical size={14}/></button>
                  ...
                  <p className="text-[10px] font-mono opacity-40 uppercase tracking-tighter mb-4">SKU: {p.sku || 'N/A'}</p>

                  {/* Stock Visualizer */}
                  <div className={`p-4 rounded-2xl flex items-end justify-between ${isLowStock ? 'bg-error/10 text-error' : 'bg-secondary/10 text-secondary'}`}>
                    <div>
                      <p className="text-[10px] uppercase font-black opacity-60">Current Stock</p>
                      <div className="flex items-baseline gap-1">
                        <span className="text-3xl font-black">{p.stock}</span>
                        <span className="text-xs font-bold uppercase">{p.unit}</span>
                      </div>
                    </div>
                    {isLowStock && <AlertTriangle size={24} className="mb-1 animate-bounce" />}
                  </div>
