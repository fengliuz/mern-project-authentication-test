...
import { LogOutIcon, PaletteIcon, Box, LayoutGrid, Warehouse, ChevronDown } from "lucide-react";

const Navbar = ({ onSelectedTheme }) => {
  const { appName, user, logout } = useAuth();
  const location = useLocation();
  
  const activeWarehouseName = localStorage.getItem("activeWarehouseName") || "Warehouse";
  const themes = ["forest", "coffee", "sunset", "dracula", "night", "synthwave", "halloween"];
  ...
  return (
    <nav className="navbar bg-secondary/80 backdrop-blur-md sticky top-0 z-50 px-4 md:px-8 shadow-lg border-b border-primary/10">
      <div className="flex justify-between items-center w-full">
        
        {/* KIRI: Logo & Info Warehouse */}
        <div className="flex items-center gap-6">
          <Link to="/" className="text-xl lg:text-2xl font-black text-base-100 drop-shadow-md">
            {appName}
          </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <div className="dropdown dropdown-end">
                ...
                <ul tabIndex={0} className="dropdown-content menu p-2 shadow-xl bg-base-100 rounded-box w-52 mt-2 border border-primary/10">
                  ...
                  <li><Link to="/select-warehouse">Manage Warehouses</Link></li>
                </ul>
              </div>

              {/* User Dropdown (Profile, Theme, Logout) */}
              <div className="dropdown dropdown-end">
                ...
                <ul tabIndex={0} className="dropdown-content menu p-2 shadow-2xl bg-base-100 rounded-box w-64 mt-3 border border-primary/10 gap-1">
                  <li className="px-4 py-3 border-b border-base-content/10 mb-2">
                    <p className="text-[10px] uppercase opacity-50 font-bold">Logged in as</p>
                    <p className="font-bold text-primary">{user.username}</p>
                  </li>

                  {/* MOBILE ONLY NAV: Muncul di dropdown saat layar kecil */}
                  <div className="lg:hidden">
                    <li className="menu-title text-[10px] uppercase opacity-50">Navigation</li>
                    <li><Link to="/category"><LayoutGrid size={18} /> Categories</Link></li>
                    <li><Link to="/product"><Box size={18} /> Products</Link></li>
                    <li><Link to="/transaction"><Warehouse size={18} /> Transactions</Link></li>
                    <div className="divider my-1"></div>
                  </div>

                  <li className="menu-title text-[10px] uppercase opacity-50">Appearance</li>
                  <div className="collapse collapse-arrow join-item border-none">
                    ...
                    <div className="collapse-title p-0 min-h-0 flex items-center gap-3 px-4 py-2 hover:bg-base-content/5 rounded-lg cursor-pointer">
                      <PaletteIcon size={18} /> Theme: <span className="capitalize">{localStorage.getItem("theme")}</span>
                    </div>
                    <div className="collapse-content p-0">
                      <ul className="bg-base-200/50 rounded-lg mt-1 max-h-40 overflow-y-auto">
                        {themes.map((t) => (
                          <li key={t}>
                            <button onClick={() => onSelectedTheme(t)} className="text-sm capitalize">{t}</button>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <li>
                    <button onClick={logout} className="text-error hover:bg-error/10 font-bold">
            ...
            <div className="flex gap-2">
              <Link to="/login" className="btn btn-sm md:btn-md btn-ghost text-primary font-bold">Login</Link>
              <Link to="/register" className="btn btn-sm md:btn-md btn-primary text-white font-bold">Register</Link>
            </div>
          )}
        </div>
      </div>
